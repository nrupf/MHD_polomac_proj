...
We assume the plasma to be quasi-neutral, i.e. that the typical plasma length scale $L$ is much greater than the screening distance $\lambda_D$, and outside of that screening distance $\frac{\rho}{ne} = \frac{n_e - n_i}{n_e} \ll 1$
...
\item Isotropic pressure and incompressible motion
    ...
    The adiabatic closure assumes that pressure is isotropic and heat is not being transported.

    Because the particles are bound to magnetic field line through gyromotion but are not bound along $\vec{B}$, 
    a more accurate model would be to assume infinite heat flow along the magnetic field lines ($\vec{B}\cdot\nabla T = 0$)

    Using mass conservation, we see that 
    \[
        \frac{d}{dt}\left(\frac{P}{\rho^\gamma}\right) = 0 \iff \frac{dP}{dt} + \gamma P \nabla \cdot \vec{V} = 0
    \]

    For incompressible motions and perfect conductivity ($\eta = 0$), we can show (see \cite{Freidberg2014}, chapter 2.6) that the two are equivalent and reduce to 
    \[
    \frac{dP}{dt} = 0
    \]

    An analysis as done in chapter 8.9 of \cite{Freidberg2014} shows that geometries with closed field lines (such as the Polomac)
    have also compressible instabilities, but the most unstable modes are always incompressible.

    For the Polomac geometry (a closed-line system), certain symmetry-preserving modes cannot be fully incompressible.
    For these modes, the quantitative stability predictions using ideal MHD should be viewed with caution, though qualitative trends remain valid.

    We assume that the motions of interest are incompressible, i.e. $\nabla \cdot \vec{V} = 0$ and equivalently $\overleftrightarrow{P}^{CM} = P$.

    In static equilibrium ($\vec{V} = 0$), this is certainly the case.
