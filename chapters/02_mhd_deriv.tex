...
\begin{itemize}
    ...
    \item Average kinetic energy density $\vec{w}_s(\vec{r},t)$: $g(\vec{v})=\frac{1}{2}m_s\vec{v}^2 \rightarrow \vec{w}_s(\vec{r},t) =\frac{1}{n_s(\vec{r}, t)} \int d\vec{v} \, \frac{1}{2}m_s\vec{v}^2 f_s(\vec{r},\vec{v},t)$
\end{itemize}
...
In order to obtain the species continuity equation, we choose $g_s(\vec{v}) = 1$.
...
(4): \quad & \int d\vec{v} \,  \left(\frac{\partial f}{\partial t}\right)_{coll} = 0 \quad \text{bec. we ass. collisions don't create or destroy particles.}
\end{align*}
...
characteristic MHD timescale $\tau_{MHD}\sim L/v_{th, i}$, where
