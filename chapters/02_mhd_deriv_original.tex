...
The following six assumptions reduce the single-fluid equations to the
ideal MHD system.  For each assumption we state the physical condition
and give its general justification; quantitative verification for the
Polomac prototype is deferred to Chapter~\ref{chap:parameters}, where
the relevant dimensionless parameters are introduced and computed.
...
\begin{itemize}
    \item Characteristic length: $L \sim \SIrange{0.04}{0.3}{\meter}$
    \begin{itemize}
        \item In the following calculations, we will in each case use the more limiting value.
    \end{itemize}
...
\begin{itemize}
    ...
    \item $\frac{L}{\lambda_D} = \frac{\SI{0.04}{\meter}}{\SI{7.4e-6}{\meter}} \approx \num{5.4e3} \gg 1$
...
Charge imbalances are screened over the Debye length
$\lambda_D = \sqrt{\epsilon_0 T_e/(e^2 n)}$.  Quasi-neutrality holds
when $L \gg \lambda_D$ and many particles populate the Debye sphere:
\[
...
Resistive diffusion is negligible when the magnetic field is frozen into
the plasma, i.e.\ when the resistive diffusion timescale greatly exceeds
the Alfv\'en timescale.  This is captured by the Lundquist number
$S \gg 1$; see Section~\ref{sec:lundquist}.

For reference, the classical Spitzer resistivity~\cite{Chen2016} is
\[
    \eta_\parallel
    = 5.2\times10^{-5}\,\frac{Z\ln\Lambda}{T_{eV}^{3/2}}
    \quad[\Omega\text{-m}].
\]
For the Polomac ($Z=1$, $T_e=100\,\text{eV}$, $\ln\Lambda=14.3$):
\[
    \eta_\parallel
    = 5.2\times10^{-5}\times\frac{1\times14.3}{(100)^{3/2}}
    \approx \SI{7.4e-7}{\ohm\meter},
\]
and since $\eta_\perp\sim2\eta_\parallel$~\cite{Chen2016} we use
$\eta\sim\SI{e-6}{\ohm\meter}$.  Whether this is small enough to justify
$\eta\to0$ is determined by $S$; see Section~\ref{sec:lundquist}.

% --- Assumption 4 ---
\item \textbf{Neglect electromagnetic waves}

The displacement current $\epsilon_0\partial_t\vec{E}$ is negligible in
Amp\`ere's law when MHD wave speeds are non-relativistic.  The
characteristic wave speed is the Alfv\'en velocity
$v_A = B/\sqrt{\mu_0\varrho}$, and the condition is
\[
    \frac{v_A}{c} \ll 1.
\]
This is verified for the Polomac in Section~\ref{sec:alfven}.
...
% --- Assumption 5 ---
\item \textbf{Small Larmor radius} ($L/\rho_i \gg 1$)

Particles are tightly bound to field lines, making the Hall term
$(1/en)\nabla\cdot\overleftrightarrow{P}_e$ negligible compared to
$\vec{V}\wedge\vec{B}$ in Ohm's law.  The relevant condition is
$L/\rho_i\gg1$ where $\rho_i = m_iV_{Ti}/(eB)$.

For the Polomac, with $V_{Ti}=1.38\times10^5\,\text{m\,s}^{-1}$ (see
Section~\ref{sec:alfven}) and $B=0.25\,\text{T}$:
\[
    \rho_i
    = \frac{1.67\times10^{-27}\times1.38\times10^5}
           {1.6\times10^{-19}\times0.25}
    = \frac{2.31\times10^{-22}}{4\times10^{-20}}
    \approx \SI{5.8e-3}{\meter}.
\]
With $L=0.04$--$0.3\,\text{m}$ this gives $L/\rho_i\approx7$--$52$.
...
% --- Assumption 6 ---
\item \textbf{Isotropic pressure and closure}

% [unchanged – same text as original on isotropic pressure,
%  adiabatic closure, incompressibility, and the note on gamma]
